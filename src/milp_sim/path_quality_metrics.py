...
def evaluate_path_quality(
    *,
    ...
    heading_jumps = [abs(wrap_to_pi(headings[i + 1] - headings[i])) for i in range(len(headings) - 1)]
    ...
    for i in range(len(headings) - 1):
        ...
        if avg_ds <= 1e-9:
            continue
        curvatures.append(wrap_to_pi(headings[i + 1] - headings[i]) / avg_ds)
...
def summarize_path_quality(metrics: Iterable[PathQualityMetrics]) -> PathQualitySummary:
    ...
    smoothness = max(0.0, 1.0 - min(1.0, 0.45 * mean_violation + 0.35 * (mean_turn / math.pi) + 0.20 * mean_jump))
