...
usage() {
    cat <<'EOF'
Usage: run_online_gui.sh [options]

Options:
  -s, --scenario-file <path>  Scenario file path (default: examples/scenario_verification_demo.json)
  -l, --log-file <path>       Log file path (default with debug: outputs/online_gui_debug_<timestamp>.log)
...
if [[ "${DEBUG_LOG}" -eq 1 ]]; then
    ...
    if [[ -z "${LOG_FILE}" ]]; then
        ...
        LOG_FILE="outputs/online_gui_debug_${stamp}.log"
    fi
fi
